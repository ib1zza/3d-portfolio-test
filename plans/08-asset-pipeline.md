...
Уточнения по факту прогона на 4.5.0:

- `flatten`, `join`, `prune`, `palette`, `instance`, `resample` и `weld` у
  `optimize` включены **по умолчанию**, перечислять их не нужно.
- Отдельной опции `--dedup` у `optimize` нет — это самостоятельная команда.
  Указание её в `optimize` роняет весь вызов с `Unknown option`.
- `simplify` выключен: у этих моделей от 5 до 33 тысяч вершин, и прореживание
  на такой сетке заметно ломает силуэт логотипов. Значение по умолчанию для
  `--simplify-error` — `0.0001`, а не `0.001`.

Фактический результат на реальных моделях проекта: **2282 КБ → 434 КБ, минус
81%**. Основной выигрыш дал не meshopt, а `weld` с индексацией: модели
экспортированы из three без индексов, поэтому каждая вершина хранилась столько
раз, сколько треугольников её использует. `simplex.glb` — 1026 КБ → 142 КБ.

Прогон автоматизирован в `scripts/optimize-models.mjs`: он же печатает таблицу
экономии и роняет сборку при выходе за бюджет 2 МБ.
